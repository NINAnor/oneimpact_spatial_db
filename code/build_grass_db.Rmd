...
all_names <- paste0("Land cover: ", c(used_names, not_used_names))
...
maps
scales <- purrr::map(strsplit(maps, "_"), ~.[3]) %>% unlist()
...
md_updated <- update_metadata(md, 
                              ...
                              unit = NA, 
                              type_data = "raster",
                              original_range_values = "ca. -1000-(+1000)", 
                              year_data = NA,
                              original_pixel_res = "???", #??
                              final_pixel_res = 50, 
                              extent = "Fenoscandia",
...
Now we load the landscape datasets from Sweden into GRASS GIS.
...
"/data/P-Prosjekter/41203800_oneimpact/sam_raw_sw/02_vector/"
